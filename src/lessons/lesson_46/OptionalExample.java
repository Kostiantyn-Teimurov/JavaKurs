...
public class OptionalExample {
    ...
    public static void main(String[] args) {
        ...
        Optional<User> optUser = getById(1);

//        System.out.println(optUser.get()); // - ошибка
        Optional<User> user2 = getById2(10);

        // boolean isPresent() - true, если есть объект
        // Мы можем проверить, есть ли в обертке значение
        if (optUser.isPresent()) {
            // Метод get() - позволяет получить значение, завернутое в Optional
            // если вызвать его на пустом (содержащим null - получим ошибку
...
//        System.out.println(user.getName());
    ...
    Optional.off(value) - создает Optional, в котором содержится значение. (не пустой/ не null)
    Если предать null - будет ошибка
