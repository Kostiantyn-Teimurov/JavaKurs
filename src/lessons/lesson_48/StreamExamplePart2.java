...
public class StreamExamplePart2 {
    ...
    - limit(long maxSize) - ограничивает кол-во эл потока. В потоке может быть не больше maxSize элементов

    - skip(long ) - отбрасывает кол-во эл слева

    - Stream<T> mapToObject(Function() mapper) - преобразование примитивного типа данных в ссылочный при помощи заданной функции

    - boxed() - преобразует поток примитивов (IntStream) в поток соответствующих оберток (Stream<Integer>)

    - mapToInt(Function) - преобразует поток Stream<Integer> в поток примитивов IntStream
   ...
   Optional<T> max(Comparator<T> comparator) - возвращает эл с макс значением (самое правое) по мнению компаратора
   Optional<T> min(Comparator<T> comparator) - возвращает эл с мин значением (самое левое) по мнению компаратора

   long count() - возвращает количество эл в потоку

   Optional<T> findFirst() - получить первый эл потока
   Optional<T> findAny() - получить случайный эл из потока
...
//                .filter(c -> c != null) // оставить в потоке только НЕ null
    ...
    private static void task12() {
        ...
        // Котов в имена, и при max найти

        Optional<String> maxNameCat = cats.stream()
...
//                .max((name1, name2) -> name1.length() - name2.length());
        ...
        if (maxNameCat.isPresent()) {
            System.out.println(maxNameCat.get());
        } else {
            System.out.println("Кота нет");
        }
...
//                .orElse("");
    ...
    private static void task14() {
        ...
        // Получить список, из 3 самых маленьких чисел
        ...
        // Получить список, отбросив 2 самых маленьких
...
//                .mapToObj(i -> i)
        ...
        // Получить из коллекции - массив примитивов
